    ...
    public void run(CommandContext cmdContext) {
        ...
        if (cmdContext.args.length == 3) {
            try {
                ...
                if (match.isPresent()) {
                    // Si le match est trouvé, on écrit les scores en temps de jeu normal (sans prolongation)
                    match.get().setScores(Integer.parseInt(args[1]), Integer.parseInt(args[2]));
                    System.out.printf("Les scores du match #%s ont été défini.\n", args[0]);
                } else {
                    System.out.printf("Aucun match n'a été trouvé avec l'ID %s.\n", args[0]);
                }
            } catch (Exception ignored) {
                System.out.println("Une erreur est survenue lors de la modification des scores.");
            }
        } else {
